...
I am going to use the same classifier that I trained for Will Kennerley's puffin samples on the SILVA-132 database. The code I used for training it was:
```
...
Need to filter for just prey using level 6 of the assignments. This is what we decided to keep:
```
qiime taxa filter-table \
  --i-table merged-table_onlymetazoa.qza \
  --i-taxonomy sklearn_taxonomy.qza \
...
This will remove any taxa that never reach an abundance of >1% in any sample.
```{r}
library(qiime2R)
library(tidyverse)
library(biomformat)
...
Downloaded the csv as "v2" as the original one had more stringent abundance filtering (removing any instance where a taxon had less than 1% abunance in a sample). This version just removed taxa that never reached more than 1% abundance in any sample.